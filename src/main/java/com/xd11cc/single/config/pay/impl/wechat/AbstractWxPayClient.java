...
import com.xd11cc.single.utils.FileUtils;
...
import static com.xd11cc.single.config.pay.impl.wechat.WxPayClientConfig.API_VERSION_V2;
import static com.xd11cc.single.config.pay.impl.wechat.WxPayClientConfig.API_VERSION_V3;

/**
 * @author xd11cc
 * @date 2026-06-08 15:17:08
...
public abstract class AbstractWxPayClient extends AbstractPayClient<WxPayClientConfig> {
    ...
    protected void doInit(String tradeType) {
        ...
        if (Objects.equals(config.getApiVersion(), API_VERSION_V2)){
            ...
            payConfig.setPrivateKeyPath(FileUtils.createTempFile(Base64.decode(config.getKeyContent())).getPath());
            ...
            if (StrUtil.isNotBlank(config.getPublicKeyContent())){
                payConfig.setPublicKeyPath(FileUtils.createTempFile(Base64.decode(config.getPublicKeyContent())).getPath());
            }
     ...
     * @return 各支付渠道的返回结果
     */
    protected abstract PayOrderRespDTO doUnifiedOrder2(PayOrderUnifiedReqDTO reqDTO)
            throws Exception;
     ...
     * @return 各支付渠道的返回结果
     */
    protected abstract PayOrderRespDTO doUnifiedOrder3(PayOrderUnifiedReqDTO reqDTO)
     ...
     * @return
     */
    protected WxPayUnifiedOrderRequest buildUnifiedOrderRequestV2(PayOrderUnifiedReqDTO reqDTO) {
        return WxPayUnifiedOrderRequest.newBuilder()
                .outTradeNo(reqDTO.getOutTradeNo())
                .body(reqDTO.getSubject())
                .detail(reqDTO.getBody())
                .totalFee(reqDTO.getPrice()) // 单位：分
     ...
     * @return 下单请求
     */
    protected WxPayUnifiedOrderV3Request buildUnifiedOrderRequestV3(PayOrderUnifiedReqDTO reqDTO) {
        WxPayUnifiedOrderV3Request request = new WxPayUnifiedOrderV3Request();
        request.setOutTradeNo(reqDTO.getOutTradeNo());
        request.setDescription(reqDTO.getSubject());
        request.setAmount(new WxPayUnifiedOrderV3Request.Amount().setTotal(reqDTO.getPrice())); // 单位：分
    ...
    protected PayOrderRespDTO doParseOrderNotify(Map<String, String> params, String body, Map<String, String> headers) throws Throwable {
        switch (config.getApiVersion()){
            case API_VERSION_V2:
                return doParseOrderNotify2(body);
            case API_VERSION_V3:
                return doParseOrderNotify3(body, headers);
            default:
                throw new IllegalArgumentException(String.format("未知的 API 版本(%s)", config.getApiVersion()));
        }
    }

    private PayOrderRespDTO doParseOrderNotify2(String body) throws WxPayException {
        // 1.解析回调
    ...
    private PayOrderRespDTO doParseOrderNotify3(String body, Map<String, String> headers) throws WxPayException {
        // 1.解析回调
    ...
    protected PayOrderRespDTO doGetOrder(String outTradeNo) throws Throwable {
        try {
            switch (config.getApiVersion()){
                case API_VERSION_V2:
                    return doGetOrder2(outTradeNo);
                case API_VERSION_V3:
                    return doGetOrder3(outTradeNo);
                default:
                    throw new IllegalArgumentException(String.format("未知的 API 版本(%s)", config.getApiVersion()));
            }
        } catch (WxPayException e) {
    ...
    private PayOrderRespDTO doGetOrder2(String outTradeNo) throws WxPayException {
        // 构建 WxPayOrderQueryRequest 对象
    ...
    private PayOrderRespDTO doGetOrder3(String outTradeNo) throws WxPayException {
        // 构建 WxPayOrderQueryV3Request 对象
    ...
    private Integer parseStatus(String tradeState) {
        switch (tradeState){
            case "NOTPAY":
            ...
            default:
                throw new IllegalArgumentException(String.format("未知的支付状态({})", tradeState));
        }
    }
    ...
    protected PayRefundRespDTO doUnifiedRefund(PayRefundUnifiedReqDTO reqDTO) throws Throwable {
        try {
            switch (config.getApiVersion()){
                case API_VERSION_V2:
                    return doUnifiedRefund2(reqDTO);
                case API_VERSION_V3:
                    return doUnifiedRefund3(reqDTO);
                default:
                    throw new IllegalArgumentException(String.format("未知的 API 版本(%s)", config.getApiVersion()));
            }
        } catch (WxPayException e) {
    ...
    private PayRefundRespDTO doUnifiedRefund2(PayRefundUnifiedReqDTO reqDTO) throws Throwable{
    ...
    private PayRefundRespDTO doUnifiedRefund3(PayRefundUnifiedReqDTO reqDTO) throws Throwable {
        // 1.构建 WxPayRefundV3Request 请求
    ...
    @Override
    protected PayRefundRespDTO doParseRefundNotify(Map<String, String> params, String body, Map<String, String> headers) throws WxPayException {
        switch (config.getApiVersion()){
            case API_VERSION_V2:
                return doParseRefundNotify2(body);
            case API_VERSION_V3:
                return doParseRefundNotify3(body, headers);
            default:
                throw new IllegalArgumentException(String.format("未知的 API 版本(%s)", config.getApiVersion()));
        }
    }

    private PayRefundRespDTO doParseRefundNotify2(String body) throws WxPayException {
        // 1.解析回调
    ...
    private PayRefundRespDTO doParseRefundNotify3(String body, Map<String, String> headers) throws WxPayException {
        // 1.解析回调
        SignatureHeader signatureHeader = getRequestHeader(headers);
    ...
    protected PayRefundRespDTO doGetRefund(String outTradeNo, String outRefundNo) throws Throwable {
        try {
            switch (config.getApiVersion()){
                case API_VERSION_V2:
                    return doGetRefund2(outTradeNo, outRefundNo);
                case API_VERSION_V3:
                    return doGetRefund3(outTradeNo, outRefundNo);
                default:
                    throw new IllegalArgumentException(String.format("未知的 API 版本(%s)", config.getApiVersion()));
            }
        } catch (WxPayException e) {
            if (ObjectUtils.equalsAny(e.getErrCode(), "RESOURCENOTEXIST", "RESOURCE_NOT_EXIST")) {
                String errorCode = getErrorCode(e);
                String errorMessage = getErrorMessage(e);
                return PayRefundRespDTO.failureOf(errorCode, errorMessage,
    ...
    private PayRefundRespDTO doGetRefund2(String outTradeNo, String outRefundNo) throws WxPayException {
        // 1. 构建 WxPayRefundQueryRequest 请求
        ...
        switch (refund.getRefundStatus()){
            case "SUCCESS":
                return PayRefundRespDTO.successOf(refund.getRefundId(), parseDateV2B(refund.getRefundSuccessTime()),
                        refund.getOutRefundNo(), response);
            case "PROCESSING":
    ...
    private PayRefundRespDTO doGetRefund3(String outTradeNo, String outRefundNo) throws WxPayException {
        // 1. 构建 WxPayRefundQueryV3Request 请求
    ...
    static String formatDateV2(LocalDateTime time) {
        return TemporalAccessorUtil.format(time.atZone(ZoneId.systemDefault()), DatePattern.PURE_DATETIME_PATTERN);
    }

    static LocalDateTime parseDateV2(String time) {
        return LocalDateTimeUtil.parse(time, DatePattern.PURE_DATETIME_PATTERN);
    }
    ...
    static String formatDateV3(LocalDateTime time) {
        return TemporalAccessorUtil.format(time.atZone(ZoneId.systemDefault()), DatePattern.UTC_WITH_XXX_OFFSET_PATTERN);
    }

    static LocalDateTime parseDateV3(String time) {
        return LocalDateTimeUtil.parse(time, DatePattern.UTC_WITH_XXX_OFFSET_PATTERN);
    }
